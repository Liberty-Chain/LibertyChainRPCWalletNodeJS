...
class Wallet {
    constructor(hostname, port, user, pass) {
        ...
        this.port = port || 48048;
        this.user = user || 'Admin';
        this.pass = pass || 'root';

        this._request('get_balance'); // This line is necessary in order to do the initial handshake between this wrapper and liberty-wallet-rpc; without it, the first request to the wrapper fails (subsequent request succeed, though.)
    }
}
...
Wallet.prototype.create_wallet = function(filename, password, language) {
    ...
    let params = {
        filename: filename || 'liberty_wallet',
        'password': password || '',
...
Wallet.prototype.restore_deterministic_wallet = function(filename, password, seed, restore_height, language, seed_offset, autosave_current) {
    ...
    let params = {
        ...
        'autosave_current': autosave_current || true,
    };
    return this._request(method, params);
};

// generate from keys
Wallet.prototype.generate_from_keys = function(restore_height, filename, address, spendkey, viewkey, password, autosave_current) {
    let method = 'generate_from_keys';
    let params = {
        'restore_height': restore_height || 0,
        filename: filename || 'liberty_wallet',
        'address': address || '',
...
// close the wallet
Wallet.prototype.close_wallet = function() {
...
// refresh the wallet
Wallet.prototype.refresh = function(start_height) {
    let method = 'refresh';
    let params = {
...
// transfer Liberty to a single recipient, or a group of recipients
Wallet.prototype.transfer = function(destinations) {
...
// get spend proof
Wallet.prototype.get_spend_proof = function(txid, message) {
    let method = 'get_spend_proof';
    let params = {
        'txid': txid || '',
        'message': message || 'this is my LBT transaction',
    };
    return this._request(method, params);
};

// check spend proof
Wallet.prototype.check_spend_proof = function(txid, signature, message) {
    let method = 'check_spend_proof';
    let params = {
        'txid': txid || '',
        'message': message || '',
        'signature': signature || '',
...
// get transfer by id
Wallet.prototype.get_transfer_by_id = function(txid) {
...
// helper function to convert Liberty amount to atomic units
